...
// переклад на укр мову опису погоди
...
$('#cityB').on('click', function(){
	...
	city=$('#city').val();
		...
		var apiURI2 = `https://api.openweathermap.org/data/2.5/forecast?q=${city}&lang=ua, uk&units=metric&appid=38242a8b4b284f623136b9543ade5ec7`;		
		$.ajax({
		  url: apiURI2,
		  dataType: "jsonp",
		  type: "GET",
	      ...
	      timeout : 500,
                    ...
                    error : function(e) {
						$('#cityC').html('<p style="color:red";>ERROR</p><p style="color:#bef7f1";>Перевірте коректність назви міста</p>');
...
var city4=$('#city').val();	
    var apiURI4 = `https://api.openweathermap.org/data/2.5/weather?q=${city4}&units=metric&appid=38242a8b4b284f623136b9543ade5ec7`;
    
	//запит даних про погоду    
    return $.ajax({
      url: apiURI4,
      dataType: "jsonp",
      type: "GET",
...
var m4 = znak.indexOf(data.weather[0].description);	    
    if (data.main.temp && data.sys) {
		...
		if (data.weather) {
			...
			document.querySelector("#demo27").innerHTML = "Погода: " + " " + znakUA[m4];
			document.querySelector("#demo125").innerHTML = "Температура: " + " " + tempMode+"°C";
			$("#tmp24").attr("src", imgURL);
			document.querySelector("#demo26").innerHTML = "Температура: " + " " + tempMode+"°C";
...
var $windText5 = $("#wind-text5");
  ...
  function formatTemperature(kelvin) {        
    ...
    //індикація індикації температури
    $tempText.html(cels);
...
console.log(lon);	
    var apiURI = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=38242a8b4b284f623136b9543ade5ec7`;
console.log(apiURI);	
//виводимо дані IP
    if (locdata){     
